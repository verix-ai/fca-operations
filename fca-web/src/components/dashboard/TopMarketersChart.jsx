...
export default function TopMarketersChart({ clients = [] }) {
  const { theme } = useTheme()
  const data = useMemo(() => {
    if (!Array.isArray(clients) || clients.length === 0) return []
    const counts = new Map()
    for (const c of clients) {
      const name = (c.director_of_marketing || 'Unassigned').trim()
  ...
  return (
    <Card className="w-full bg-hero-card border surface-main">
      <CardHeader className="p-6 pb-4 border-b border-white/5">
        <CardTitle className="text-heading-primary">Top 5 Marketers by Clients</CardTitle>
      </CardHeader>
          ...
          <div className="w-full h-56">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data} margin={{ top: 8, right: 8, bottom: 8, left: 0 }}>
                <defs>
                  <linearGradient id="marketerGradient" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#63FF82" />
                    <stop offset="100%" stopColor="#00B9FF" />
                  </linearGradient>
                </defs>
                <CartesianGrid strokeDasharray="3 3" stroke={gridColor} />
                <XAxis dataKey="name" tick={{ fill: axisTickColor, fontSize: 12 }} axisLine={false} tickLine={false} interval={0} angle={-18} textAnchor="end" height={40} />
                ...
                />
                <Bar dataKey="value" fill="url(#marketerGradient)" radius={[12, 12, 4, 4]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
