...
function buildNavigationSections(user) {
  ...
  const managementItems = isMarketer
    ? [
        { id: 'marketer-intake', title: 'Referral Form', description: 'Capture marketer onboarding details', icon: ClipboardList, url: createPageUrl('MarketerIntake') },
        { id: 'clients', title: 'Clients', description: 'Your clients and status', icon: Users, url: createPageUrl('ClientList') },
        { id: 'prospects', title: 'Prospects', description: 'Your referrals and status', icon: Users, url: createPageUrl('Prospects') },
        { id: 'reports', title: 'Reports', description: 'Insights and performance metrics', icon: PieChart, url: createPageUrl('Reports') },
        { id: 'messages', title: 'Messages', description: 'Team and client communication hub', icon: MessageSquare, url: createPageUrl('Messages') },
      ]
    : [
        { id: 'dashboard', title: 'Operation Board', description: 'Control center for daily activity', icon: BarChart3, url: createPageUrl('Dashboard') },
        { id: 'prospects', title: 'Prospects', description: 'All marketers’ referrals', icon: Users, url: createPageUrl('Prospects') },
        { id: 'reports', title: 'Reports', description: 'Insights and performance metrics', icon: PieChart, url: createPageUrl('Reports') },
        { id: 'client-intake', title: 'New Client Intake', description: 'Add and triage new client requests', icon: Plus, url: createPageUrl('ClientIntake') },
        { id: 'marketer-intake', title: 'Referral Form', description: 'Capture marketer onboarding details', icon: ClipboardList, url: createPageUrl('MarketerIntake') },
        { id: 'clients', title: 'All Clients', description: 'Directory of managed client accounts', icon: Users, url: createPageUrl('ClientList') },
        { id: 'messages', title: 'Messages', description: 'Team and client communication hub', icon: MessageSquare, url: createPageUrl('Messages') },
      ]
  ...
  const base = [
    ...
    },
    {
      ...
      items: [
        ...
        ...(!isMarketer ? [{ id: 'settings', title: 'Settings', description: 'Preferences and workspace controls', icon: Settings, url: createPageUrl('Settings'), placement: 'footer' }] : []),
      ],
    },
  ]
  return base
          ...
          <div key={section.id} className="flex w-full flex-col gap-3">
            ...
            <div className="flex flex-col gap-2">
              {section.id === 'workspace' && (
                <div className="flex items-center gap-3 rounded-xl border border-[rgba(var(--border),0.35)] bg-[rgba(var(--bg),0.78)] px-4 py-2 text-sm font-medium text-[rgba(var(--muted),0.75)] shadow-[0_12px_24px_-20px_rgba(0,0,0,0.45)]">
                  ...
                  <span className="flex flex-col text-left">
                    ...
                    <span className="text-xs font-normal text-[rgba(var(--muted),0.6)]">Light / Dark</span>
                  </span>
                </div>
              )}
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    classNames(
                      ...
                      isActive &&
                        'border-[rgba(var(--brand),0.45)] bg-[rgba(var(--border),0.3)] text-[rgb(var(--text))] shadow-[0_18px_40px_-30px_rgba(0,0,0,0.7)]',
                    )
                  }
                  title={item.title}
                >
                  ...
                  <span className="flex flex-1 flex-col text-left">
                    ...
                    <span className="text-xs font-normal text-[rgba(var(--muted),0.65)]">{item.description}</span>
                  </span>
...
export default function Layout({ children, currentPageName }) {
  ...
  const { user, logout, loginWithPassword } = authValue
            ...
            <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Button
                  variant="outline"
                  borderRadius="999px"
                  className={`px-3 py-1 text-xs ${user?.role === 'admin' ? 'border-[rgba(var(--brand),0.55)]' : ''}`}
                  onClick={() => loginWithPassword({ email: 'admin@fca.local' })}
                ...
                <Button
                  variant="outline"
                  borderRadius="999px"
                  className={`px-3 py-1 text-xs ${user?.role === 'marketer' ? 'border-[rgba(var(--brand),0.55)]' : ''}`}
                  onClick={() => loginWithPassword({ email: 'marketer@fca.local' })}
                  title="Switch to Marketer"
                >
                  Marketer
                </Button>
                <Button
